...
Node *huffman_build_tree(uint64_t hist[static ALPHABET]) {
...
void huffman_build_code(Node *root, Code table[static ALPHABET]) {
    static Code c;
    ...
    if (root) {
        ...
        if (!root->left && !root->right) {
            ...
            code_push_bit(&c, 0);
            huffman_build_code(root->left, table);
            code_pop_bit(&c, &bit);

            code_push_bit(&c, 1);
            huffman_build_code(root->right, table);
            code_pop_bit(&c, &bit);
        }
    }
}

Node *huffman_rebuild_tree(uint16_t tree_size, uint8_t tree[static tree_size]) {
...
void huffman_delete_tree(Node **root) {
    if (*root) {
...
void huffman_print_tree(Node *root) {
