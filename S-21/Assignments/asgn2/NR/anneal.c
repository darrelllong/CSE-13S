...
#define TFACTR 0.9
#define ALEN(a, b, c, d)                                                       \
  sqrt(((b) - (a)) * ((b) - (a)) + ((d) - (c)) * ((d) - (c)))

void anneal(x, y, iorder, ncity) float x[], y[];
int iorder[], ncity;
{
...
float revcst(x, y, iorder, ncity, n) float x[], y[];
int iorder[], ncity, n[];
{
  float xx[5], yy[5], de;
  int j, ii;
...
void reverse(iorder, ncity, n) int iorder[], ncity, n[];
{
  int nn, j, k, l, itmp;
...
float trncst(x, y, iorder, ncity, n) float x[], y[];
int iorder[], ncity, n[];
{
  float xx[7], yy[7], de;
  int j, ii;
...
void trnspt(iorder, ncity, n) int iorder[], ncity, n[];
{
