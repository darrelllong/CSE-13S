...
void rkqc(y, dydx, n, x, htry, eps, yscal, hdid, hnext, derivs) float y[],
    dydx[], *x, htry, eps, yscal[], *hdid, *hnext;
...
int n;
{
  ...
  for (;;) {
    ...
    *x = xsav + h;
    if (*x == xsav)
      nrerror("Step size too small in routine RKQC");
    ...
    if (errmax <= 1.0) {
      *hdid = h;
      *hnext =
          (errmax > ERRCON ? SAFETY * h * exp(PGROW * log(errmax)) : 4.0 * h);
      break;
    }
