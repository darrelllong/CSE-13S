...
void make_prime(mpz_t p, uint64_t bits, uint64_t iters) {
    mpz_t temp, mod, minimum;
    mpz_inits(temp, mod, minimum, NULL);
    mpz_set_ui(temp, 1);
    mpz_mul_2exp(minimum, temp, bits - 1); // minimum = 2^{bits - 1}

    do {
        do {
            mpz_urandomb(p, state, bits); // Guess a (random() % 2^{bits - 1})
        } while (mpz_even_p(p) || mpz_cmp(p, minimum) < 0); // Skip even and too small numbers
    } while (!is_prime(p, iters)); // Until we find a prime

    mpz_clears(temp, mod, minimum, NULL); // Free local variables
}
