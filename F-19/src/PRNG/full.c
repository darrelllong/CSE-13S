...
uint128_t randomSPECK(uint64_t entropy[], uint64_t key[]) {

  uint64_t out[2] = {0};
  uint128_t result;

  Speck128ExpandKeyAndEncrypt(entropy, out, key); // Encrypt 16 bytes
...
int main(int argc, char **argv) {
  uint64_t key    [2] = {0xb65d5bff1c9cd747, 0x0536d9ce768574c2}; // Dr Thompson
  uint64_t entropy[2] = {0xb1a84340c5fda1e5, 0x4b7049f8cb1b738a}; // Dr Johnson
  ...
  while ((c = getopt(argc, argv, "-kqtn:c:")) != -1) {
    switch (c) {
    ...
    case 'c': { // Set the counter
       entropy[0] = strtoll(optarg, (void *) 0, 10);
       entropy[1] = 0;
       break;
    }
    case 'k': { // Read /dev/random
       int r = open("/dev/random", O_RDONLY);
       ret = read(r, key, 16);
       if(ret < 0){
           printf("Could not read from /dev/random %d\n", ret);
       }
       ret = read(r, entropy, 16);
       if(ret < 0){
           printf("Could not read from /dev/random %d\n", ret);
       }
       break;
    ...
    case 't': { // Seed with current time
      ...
      Speck128ExpandKeyAndEncrypt(entropy, entropy, key); // Spread the counter
      break;
    }
  ...
  for (uint64_t i = 0; i < tests; i += 1) {
    uint128_t r = randomSPECK(entropy, key);
    // Tally bits
