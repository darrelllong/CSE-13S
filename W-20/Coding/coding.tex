...
\text{\#define}. Enumerations should be \texttt{typedef}'d. If
...
//
// Definition of a BitVector, a dynamically allocated vector of bits.
//
...
//
// length:     The length of the BiVector in bits.
//
...
//
// Sets a bit to one in a BitVector.
//
// bv:     The BitVector.
...
//
// Clears a bit to zero in a BitVector.
//
// bv:     The BitVector.
...
\begin{lstlisting}[title=bv.c]
// bv.c - Contains the implementation of the BitVector ADT.

#include <assert.h> // Asserts that a statement is true.
#include <inttypes.h>
#include "bv.h"
...
BitVector *bv_create(uint32_t length) {
  ...
  assert(bv); // Asserts that malloc didn't fail to allocate memory.
  bv->length = length;
...
// The other BitVector functions you will implement yourself.
\end{lstlisting}
\end{document}
